...
def load_users():
    if not os.path.exists(USERS_FILE):
        default = {"admin": {"password": "abdouuu100", "role": "abdouuu"}}
        save_users(default)
        return default
    with open(USERS_FILE, 'r', encoding='utf-8') as f:
        data = json.load(f)
    if 'admin' not in data:
        data['admin'] = {"password": "abdouuu100", "role": "abdouuu"}
        save_users(data)
    return data
...
def create_default_files(server_dir):
    ...
    if not os.path.exists(main_py):
        with open(main_py, 'w', encoding='utf-8') as f:
            f.write('''# 𝗔𝗕𝗗𝗢𝗨𝗨𝗨 𝗩𝗜𝗣 𖣘 HOSTING - Default Bot
import time

print("=" * 40)
print("Bot is running on 𝗔𝗕𝗗𝗢𝗨𝗨𝗨 𝗩𝗜𝗣𖣘 HOSTING")
...
def api_create_server():
    ...
    if not username:
        username = f"IshrakShadman{random.randint(10000, 99999)}"
...
if __name__ == '__main__':
    print("\n" + "=" * 50)
    print("🚀 𝗔𝗕𝗗𝗢𝗨𝗨𝗨 𝗩𝗜𝗢𖣘 HOSTING - FINAL")
    print("=" * 50)
